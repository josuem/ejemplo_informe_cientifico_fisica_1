...
Para definir un plano inclinado se emplea un soporte universal junto a un riel Pasco, como se muestra en la Figura @fig-montaje. El riel se fija al soporte mediante una nuez, estableciendo inicialmente un ángulo de 10° medido con un transportador analógico (rango 0–180°, resolución de 1°). Sobre el riel se posiciona un smartphone con la aplicación *Phyphox*, que utiliza su acelerómetro para registrar los ángulos de inclinación ($\alpha$) y rotación ($\beta$) del plano.
...
Utilizando las Ecuaciones @eq-promedio y @eq-incertidumbre, se obtiene el valor promedio y la incertidumbre asociada para cada componente:
$$
\begin{aligned}
